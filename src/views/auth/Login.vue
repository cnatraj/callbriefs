...
const handleSubmit = async () => {
  const { error: err } = await auth.signInWithEmail({
    email: email.value,
    password: password.value,
  });
  if (err) return;
  await org.loadMemberships();
  router.push(org.isOnboarded ? "/briefs" : "/onboarding");
};
...
<template>
  <div class="min-h-screen bg-bg relative" data-screen-label="Login">
    <!-- Top bar -->
    <header class="flex items-center justify-between px-8 py-7">
      ...
      <div class="flex items-center gap-3">
        <span class="text-ink-500">Don't have an account?</span>
        <router-link
          to="/signup"
    ...
    <main class="flex justify-center px-6 pt-6 pb-16">
      ...
      >
        <div class="text-center mb-6">
          ...
          <p class="text-ink-500 mt-2">Sign in to your CallBriefs workspace.</p>
        </div>
        ...
        <form class="flex flex-col gap-5" @submit.prevent="handleSubmit">
          ...
          <div class="flex flex-col gap-2">
            <div class="flex items-center justify-between">
              ...
              >
              <a
                class="text-[13px] text-ink-500 hover:text-ink-900 cursor-pointer"
              >
                Forgot password?
              </a>
            </div>
          ...
          <!-- Keep me signed in -->
          <label
            class="flex items-center gap-[10px] cursor-pointer select-none"
          >
            <span
              class="w-[18px] h-[18px] grid place-items-center rounded-xs transition-colors"
              :class="
                keepSignedIn
              ...
              "
            >
              <IconCheck v-if="keepSignedIn" :size="12" :sw="3" />
            </span>
            <input v-model="keepSignedIn" type="checkbox" class="sr-only" />
            <span class="text-ink-900">Keep me signed in on this device</span>
          </label>
