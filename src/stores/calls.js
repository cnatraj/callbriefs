...
export const useCallsStore = defineStore("calls", () => {
  ...
  const loadCall = async (callId) => {
    ...
    const { data, error: err } =
      await callsService.getCallWithMicrosite(callId);
    if (err) {
      error.value = err.message;
      activeCall.value = null;
      // Mark the id as attempted so Detail.vue's isMissing fires on
      // invalid-UUID / not-found errors rather than rendering blank.
  ...
  const needsPolling = computed(
    () =>
      !!activeCall.value && NON_TERMINAL_STATUSES.has(activeCall.value.status),
  );
